...
<article id="post-<?php the_ID(); ?>" class="post clearfix style-reform">
	...
	<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute( array( 'before' => 'リフォーム事例「', 'after' => '」詳細ページへ' ) ); ?>" class="thumbnail">

		<span class="attachment">
			<?php if ( function_exists('the_post_image') ) {
				if ( the_post_image('thumbnail') === false ){
					?><img src="<?php echo get_template_image('noimage');?>" alt="<?php the_title_attribute( array( 'before' => 'リフォーム事例「', 'after' => '」詳細ページへ' ) ); ?>" /><?php
				}
			} ?>
		</span>
...
<div class="metabox">
		...
		<span class="todetail">
		<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title(); ?>">詳しく見る</a>
		</span>
</div><!--metabox-->
